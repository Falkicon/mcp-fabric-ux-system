...
| DATA-UPDATE-FRONTMATTER| Add Missing Frontmatter to Component Docs | M               | To Do    | Add `id`, `title`, `area` to files in `_docs_fabric_ux/components/` for indexing.  |
| DATA-IMPROVE-CHUNKING | Optimize Documentation Format for Better RAG Chunking | L               | Future    | Implement the following structural improvements to documentation for better RAG accuracy:<br>1. **Component IDs in Section Headers**: Add component name to all section titles (e.g., "Button Attributes" instead of just "Attributes").<br>2. **Section Markers**: Add `<!-- BEGIN-SECTION: xxx -->` and `<!-- END-SECTION: xxx -->` markers around logical content blocks to ensure proper chunking.<br>3. **Enhanced Metadata**: Add component-specific metadata in frontmatter like `component_type`, `primary_component`, `inheritable`, `inherits_from`, and `sections` array.<br>4. **Property Grouping**: Organize related properties with clear comment separators (e.g., `<!-- Appearance Attributes -->`, `<!-- State Attributes -->`).<br>5. **Section References**: Add `<div>` containers with unique IDs and data attributes (e.g., `<div id="fabric-button-attributes" data-component="fabric-button" data-section="attributes">`).<br>Create a standard template incorporating these improvements for all component documentation. |
| **IMPL**  | **Core Implementation (RAG)**                         |                  |           | *Depends on DATA tasks*                                                                                  |
| IMPL-01 | Implement Indexing Script (`scripts/indexDocs.ts`)    | L                | Pending Validation | Target `_docs_fabric_ux`. Indexes content into ChromaDB via HTTP. Requires ChromaDB server (e.g., Docker). `npm run index-docs`. Needs re-run after DATA-UPDATE-FRONTMATTER. |
| IMPL-02 | Implement `askFabricDocs` MCP Tool                  | M                | Pending Validation | Connect to Vector DB (ChromaDB Server), embed query, search, return chunks. Needs validation after re-indexing. |
| IMPL-03 | Integrate Tool into MCP Server (`src/index.ts`)       | S                | Pending Validation | Registered tool via `server.tool()`. Needs validation after re-indexing.                     |
...
| TEST-03 | Run Indexing Script after Frontmatter Update          | S                | To Do     | `npm run index-docs`. Depends on DATA-UPDATE-FRONTMATTER.                                                   |
| TEST-04 | Validate `askFabricDocs` via MCP Inspector          | S                | To Do     | Test tool execution and result relevance after re-indexing. Depends on TEST-03.                             |
